...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    @Query(value = " select o.*  from orders o, order_details od where o.order_id=od.order_id and o.user_id =?1 and od.variant_id in(select variant_id from variants v where v.product_id = ? 2)", nativeQuery = true)
