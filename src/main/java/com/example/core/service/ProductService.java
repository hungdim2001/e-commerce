...
public class ProductService {
    ...
    public ProductDTO create(Long id, MultipartFile thumbnail, MultipartFile[] images, String[] oldImages, Long productTypeId, String name,
                             Boolean status, MultipartFile description, String[] productCharValues, Variant[] variants, MultipartFile[] variantImages) throws Exception {
        ...
        if (!Utils.isNull(id)) {
            ...
            List<String> newImagesVariant = Arrays.stream(variants).map(variant -> {
            ...
            if (!Utils.isNullOrEmpty(variantImages)) {
                //save variant Image
                Arrays.asList(variantImages).forEach((image ->
                {
            ...
            {
                if (!Utils.isNull(variantImageMap.get(variant.getChars()))) {
